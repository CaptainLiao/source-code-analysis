...
````
如果 cb 是一个对象，那么 return createWatcher()。这是因为定义在 VUE 原型上的 $watch 方法可以被用户直接调用，传入的 cb 可以是任意类型(函数、对象、字符串)。
...
现在，我们知道了 lazy watcher 和 user watcher。它们之间有什么联系或者说区别？VUE 中还有没有其他 watcher？
