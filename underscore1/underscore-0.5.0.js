...
(function () {
  ...
  // 如果 Underscore 作为函数来调用（即：_(obj)），则返回一个 wrapper 构造函数的实例
  // wrapper 函数作为构造器，持有 underscore 的所有功能/方法
   ...
   * 核心的 each 方法实现
   * 
   * @param {Object} obj          需要遍历的对象：实现了 forEach 方法的对象，数组和原生对象
   * @param {Function} iterator   遍历器：三个参数：currentValue, index, array
   * @param {Object} context      上下文：如有，iterator中的 this 就指向这个context
   * @returns obj                 返回原始对象
   */
   ...
   * 聚合obj对象的值并返回一个结果值
   * 
   * @param {Array} obj 聚合对象
   * @param {Array} memo 用于初始化
   * @param {Function} iterator 聚合函数：previousValue，nextValue,currentIndex, array
   * @param {Object} context 绑定 iterator 的this指向
   * @returns memo 聚合后结果值
   */
